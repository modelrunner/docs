...
| Tool | Purpose |
| --- | --- |
| `run_model` | Submits an async inference request. Returns `requestId` immediately. Input field for files must be a URL — use `upload_file`. Optional `metadata` (flat string map, max 16 keys) tags the request for later filtering. |
...
<Tip>
  See [Build a wrapper](/guides/wrappers/authoring) for a full worked example of this flow — from reading a base model's schema to publishing a live endpoint.
</Tip>
