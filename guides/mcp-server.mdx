...
<Tabs>
  <Tab title="Claude Desktop">
    Claude Desktop and claude.ai take a remote MCP server as a **custom connector**, not as a `claude_desktop_config.json` entry (that file launches local stdio servers only).
    ...
    <Steps>
      ...
      <Step title="Paste the server URL">
        Enter `https://mcp.modelrunner.run/mcp` and confirm.
      </Step>
    </Steps>

    Sign in to ModelRunner when the authorization window opens. Custom connectors are subject to your Claude plan's own limits. If your client only speaks stdio, use the `mcp-remote` bridge below instead.
  </Tab>
...
- **Server never appears in Claude Desktop** — a `url` entry in `claude_desktop_config.json` is ignored without an error; that file only launches local stdio servers. Add the server as a custom connector (see the Claude Desktop tab above) or bridge it with `mcp-remote`.
