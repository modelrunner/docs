...
The `X-Fal-Object-Lifecycle-Preference` and `X-Fal-Store-IO` headers are accepted as aliases with identical behaviour, so existing code keeps working. If you send both spellings, the `X-Modelrunner-*` one wins.

The one deliberate difference is `initial_acl`, which ModelRunner does not implement — see [above](#access-controls-are-not-supported).