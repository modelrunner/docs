...
<Note>
  For a [multipart upload](/guides/file-uploads#multipart-upload-large-files), the preference rides on `POST /storage/upload/complete` instead of the initiate — that is the call that creates the file's row.

  Both clients handle that for you, so a large upload keeps its expiration: pass `lifecycle` to `upload`, `upload_file` or `upload_image` on `modelrunner-ai` **0.6.0 or newer**, or to `storage.upload()` on the JavaScript client **1.3.0 or newer**. Both switch to multipart above 90 MB. On earlier versions the preference did not survive a large upload — the JavaScript client accepted the option and dropped it, and Python had no upload option at all — so upgrade, or drive the raw HTTP flow and set the header on the complete call yourself.
</Note>
