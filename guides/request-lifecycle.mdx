...
Metadata is stored on the request and returned on its status, result, and list payloads — it is **never sent to the model** and never merged into the stored `input`.
