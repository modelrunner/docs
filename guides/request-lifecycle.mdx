...
| `CANCELLED` | The request was cancelled before it finished. Reachable on [Serverless GPU deployment](/guides/serverless-gpus/cancellation) jobs only. Not billed. |
...
<Note>
  ...
  **`CANCELLED` applies to deployment jobs only.** For catalog models and wrappers nothing sets it, and `cancel_url` does not cancel (see [Cancellation](#cancellation)). Jobs submitted to your own [Serverless GPU deployment](/guides/serverless-gpus/cancellation) *can* reach it — so a poller shared across both must treat `CANCELLED` as terminal.
</Note>
...
| `cancel_url` | Cancels the request on a deployment endpoint; returns the current status on a model or wrapper endpoint. See [Cancellation](#cancellation). |
...
`PUT` (or `GET`) `/{ownerName}/{modelName}/requests/{requestId}/cancel` — what it does depends on what the endpoint is.

**On a Serverless GPU deployment, cancel is real.** The job is stopped, the request lands on terminal `CANCELLED` with `billingStatus: "failed"`, and a `completed` webhook fires. See [Cancel and terminal states](/guides/serverless-gpus/cancellation).

<Warning>
  **On a catalog model or wrapper, this endpoint does not cancel anything.** It is wired to the same handler as `status_url` and returns the same response, so the request continues to run and will still be billed if it completes.

  The route exists so that clients and SDKs built against it keep working once real cancellation ships for catalog models too. Until then, treat a submitted model request as uninterruptible and do not rely on `cancel_url` to stop billable work.
</Warning>
