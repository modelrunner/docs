...
The fastest way to build a wrapper is to let an AI assistant do it for you. Connect the [MCP server](/guides/mcp-server) and ask — the assistant has tools to read the base model, draft your wrapper, dry-run the transform, and publish it, all from chat. This page walks one wrapper from idea to live endpoint.

<Note>
  You can also build wrappers in the **web wrapper studio** — a point-and-click form on [modelrunner.ai](https://modelrunner.ai) with the same fields as below and a live preview. It's the friendliest path for non-developers; the AI-assistant flow here is the most reproducible and is what this guide teaches. (The studio UI evolves, so we keep its walkthrough short — the underlying concepts are identical.)
</Note>
...
<Tip>
  You never paste `ownerName` or pricing into these tools. Ownership is derived from your authenticated identity, and markup is server-owned. The assistant only authors the *design* of the wrapper.
</Tip>
