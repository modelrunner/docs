...
<Step title="Callers can override with `base_model`">
  To target a non-default model, the caller sends a top-level `base_model` field in the request body containing that model's **id** — the UUID `get_wrapper` lists for each of the wrapper's base models (not the model's `ownerName/alias` endpoint):
  ...
  { "subject": "a fox in a trench coat", "mood": "noir", "base_model": "<model-uuid>" }
  ```

  `base_model` is **honored only when the wrapper has more than one base model**, and the id must be one of the wrapper's own base models — otherwise the request is rejected. For a single-model wrapper it's ignored.
</Step>
