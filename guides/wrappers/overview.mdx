---
title: "Wrappers"
description: "Turn a raw model into a purpose-built product — your own inputs, a baked-in prompt recipe, and your own pricing — callable at a single endpoint."
...
- **field mappings** — rules that reshape your inputs into each base model's native schema,
- and **markup pricing** — your price on top of the base model's cost.
...
<Columns cols={2}>
  ...
  <Card title="Resell with markup" icon="circle-dollar-to-slot">
    Apply a percentage markup over the base model's price. Your wrapper bills at the marked-up rate; the difference is yours.
  </Card>
...
```text
        caller's input                wrapper                       base model
   ┌──────────────────────┐    ┌────────────────────────┐    ┌────────────────────┐
   │ subject: "a fox"      │    │ 1. prompt template  ───┼──► │ prompt: "a fox,    │
   │ mood:    "noir"       │──► │ 2. field mappings      │    │   high-contrast…"  │
   │ (base_model?)         │    │ 3. markup applied      │    │ num_outputs: 1     │
   └──────────────────────┘    └────────────────────────┘    └────────────────────┘
```
...
| **Markup** | A percentage uplift over the base model's price. Server-owned. | Below |
...
A wrapper bills at the base model's price **plus your markup**. If a base model costs `$0.10` per output and the wrapper's markup is `20%`, each call settles at `$0.12`. Markup is applied at request time and snapshotted onto the request, so billing is self-contained and unaffected by later price changes.

Markup is **server-owned** — you set it from the wrapper studio (or it's configured by ModelRunner), not in the create payload. New wrappers start at `0%`.
