...
const result = await modelrunner.run("my-function-id", {
  input: { prompt: "A lighthouse on a cliff at sunrise" },
});
...
<ListTable>
  ...
  <Body>
    <Row>
      <Cell>Node.js 18+</Cell>
      <Cell>Full</Cell>
      <Cell>Recommended for servers and CLIs</Cell>
    </Row>
    <Row>
      <Cell>Vercel/Edge runtimes</Cell>
      <Cell>Full</Cell>
      <Cell>Keep credentials in environment variables</Cell>
    </Row>
    <Row>
      <Cell>Browsers</Cell>
      <Cell>Via proxy</Cell>
      <Cell>Do not embed secrets client-side</Cell>
    </Row>
    <Row>
      <Cell>React Native</Cell>
      <Cell>Via proxy</Cell>
      <Cell>Call a secure backend route</Cell>
    </Row>
  </Body>
</ListTable>
...
- [Model example](https://modelrunner.ai/bytedance/sdxl-lightning-4step)
